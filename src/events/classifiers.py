...
class EventClassifier:
    ...
    def default():
        ...
        question_df["класс"] = [
            Event.INFORMATION.value for _ in range(question_df.shape[0])
        ]
        question_df["реплика"] = question_df["question"]
        df = pd.concat([df, question_df])

        vectorizer = TfidfVectorizer()
        vectorizer.fit(df["реплика"])
        vectorizer = SentenceTfIdfVectorizer(vectorizer)

        replica_container = ReplicasContainer()
        replica_container.extend(
            df["реплика"], list(map(Event.from_number, df["класс"]))
        )
        event_classifier = EventClassifier(
            replica_container, sentence_vectorizer=LaserVectorizer()
        )
